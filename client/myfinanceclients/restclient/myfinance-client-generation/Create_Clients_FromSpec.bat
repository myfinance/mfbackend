...
set MAVEN_OPTS=-DsupportingFiles -Dapis=Hello,MyFinance,MyFinanceRunner -Dmodels

echo %MAVEN_OPTS%

set generatorGeneral=exec:java -Dexec.mainClass="io.swagger.codegen.SwaggerCodegen"  -Dexec.classpathScope=runtime -Dexec.cleanupDaemonThreads=false

call mvn %generatorGeneral%  -Dexec.args="generate  -t %~dp0..\myfinance-client-generator\templates\MyFinanceClient_Java --library jersey1 --additional-properties serializableModel=true --artifact-version %version%  -l de.hf.dac.myfinance.codegen.MyFinanceClient_JavaGenerator -o %generatedroot%/myfinance-client-generated-java/ -i %specfile%"
