...
Der Businesskey muss nicht unique sein. Bei einem Buasparvertrag z.b. steht dort die gleiche Vertragsnummer wie bei dem entsprechenden Bausparkonto und dem Darlehen.
So können die einzelnen Teile auch schneller selektiert werden
...
Bei den Portfolien gibt es zwei Ausnahmen. Zu einen das Depot, da Position imme aus den Trades berechnet werden und nicht in der DB abgelegt werden, hänge alle Realen Werte von Positionen am Depot.
 ...
 * defaultBudget: Einem Account(ancestor) kann genau ein DefaultBudget zugewiesen werden. Jedes Budget darf mehrfach mit diesem Edgetype vorkommen (als descendant)
 wird verwendet als StandardBudget zur Verbuchung von regelmäßigen Einnahmen/Ausgaben z.B. <<Loan>>
 * Analyse Portfolien: hier sollen die beziehungen von artificial Portfolien und deviersen Accounts und Positionen abegildet werden (Soll noch nicht umgesetzt werden)
 * Tenent-Graph: Abbildung des realen und virtuellen Vermögens eines Mandanten <<Tenant>>
...
rectangle Tenant
rectangle BudgetGroupPortfolio
rectangle AccountPortfolio
rectangle BudgetsGroup1
rectangle BudgetsGroup2
rectangle Budget1
rectangle Budget2
rectangle Budget3
rectangle Depot1
...
Tenant --> BudgetGroupPortfolio
Tenant --> AccountPortfolio
BudgetGroupPortfolio --> BudgetsGroup1
BudgetGroupPortfolio --> BudgetsGroup2
BudgetsGroup1 --> Budget1
BudgetsGroup1 --> Budget2
BudgetsGroup2 --> Budget3
...
Wiederkehrende Cashflowswerden in einer Tabelle Konfiguriert. Die Konfigurationen werden hier ausnahmsweise historisiert, da es von interesse ist, wie sich manche Dauerbuchungen entwickelt haben
...
recurrenttransaction : Date validfrom
recurrenttransaction : Date validto
...
recurrentfrequence= Monthly, Quaterly, Yearly in der DB aber nur als id abgebildet, sa dass die Software ohne DB-Änderungen erweitert werden kann
recurrencytype= Einnahme/Ausgabe oder Transfer -> wie recurrentfrequence nur als ID abgebildet. Benötigt um zu wissen ob auf den Instrumenten jeweils ein positiver oder negativer cashflow fliesst
...
|BudgetGroupPortfolio |portfolio | Ein Portfolio mit allen Budget-Gruppen eines Mandanten
...
==== BudgetGroupPortfolio

beinhaltet alle Budget-Gruppe eines Mandanten. Dies ist eigentlich ein ganz normales Portfolio. Der Typ ist nur benötigt, da jeder Mandant genau ein BudgetGroupPortfolio und ein AccountPortfolio haben muss die den geleichen Wert besitzen sollen
...
investitionen in das haus(zb wohnungsausbau) werden als kredit von mandanten zu sich selber dargestellt dass das haus mit zins und tilgung zurückzahlt. Dies wird aber nicht extra in der Datenbank abgelegt sondern nur in der Berechnung des wertes und der rendite der oben genannten sicht beruecksichtigt.
alle sonstigen kosten und rücklagen werden über budget geglättet -> alle hausbudget spielen keine rolle für den hauswert bzw dessen rendite
